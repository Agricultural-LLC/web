...
const normalizeApiPath = (path) => path.replace(/^\/api(?=\/|$)/, "") || "/";
...
exports.api = onRequest({ cors: false }, async (req, res) => {
  ...
  if (apiPath.startsWith("/link-preview")) {
    return handleLinkPreview(req, res);
  } else if (apiPath.startsWith("/cms/sync")) {
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        res
          .status(403)
          .json({ error: "Access to private networks is not allowed" });
        return;
      }
    ...
    try {
      const response = await fetch(normalizedUrl, {
        method: "GET",
        headers: {
          "User-Agent": "Mozilla/5.0 (compatible; remono-LinkPreview/1.0)",
          Accept:
